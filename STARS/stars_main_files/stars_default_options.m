...
stars_option.DisplayOutputs = 1;    % 1 to display #iteration, #evaluations, function values and trust-region radius during optimization process; 0 otherwise
...
stars_option.FiniDiffParam = 0.01;  % Forward finite difference parameter (default value). Updated in the algorithm via the trust-region radius
...
stars_option.LowerBounds = [];      % Lower bounds on the problem variables; set to -Inf in the algorithm if no bound is provided
stars_option.MaxFuncEval = Inf;     % Maximum number of function evaluations (handled by MCestimate.m)
stars_option.MaxNumberIters = 1000; % Maximum number of iterations (considered only when stars_option.MaxFuncEval = Inf)
...
stars_option.SeedValue = [];        % Seed value when stars_option.FixSeed = 1; equals floor(1+ pi) in the algorithm if no value is provided
stars_option.SolutionFile = 0;      % 1 to generate a .txt file containing the solution of the problem; 0 otherwise (see [4] below)
...
stars_option.SubspaceMatrix = 0;    % 0 for Gaussian, 1 for Hashing, 2 for identity matrix (STORM algorithm): matrices defining the subspaces
stars_option.UpperBounds = [];      % Upper bounds on the problem variables; set to +Inf in the algorithm if no bound is provided
stars_option.UsePreviousSamples = 1; % 1 to use available samples in cache for estimates computation; 0 otherwise
