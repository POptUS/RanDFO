function [objective, X0, n, npmax, nfmax, gtol, delta, nfs, m, F0, xkin, L, U, printf, spsolver, hfun, combinemodels] = setup_yatsop_test(mode,probs)
addpath('~/YATSOp/m/');
...
% mode should be largescale or midscale
% probs should be a range between 1 and 38

if strcmp(mode,'midscale')
Var = [1 100 200 1e-7 % ARGLALE
    2    100 200 5e-8 % ARGLBLE
    3    100 200 5e-8 % ARGLCLE  (new)
    201  100 100 4e-9 % ARTIF
    202  100 198 1e-8 % ARWHDNE %100 198
    ...
    5    100 198  1e-8 %100 198 1e-8 % ROSENBR  (new)
    213  100 164 3e-8 % SPMSQRT
    121  100 100 4e-9 % SROSENBR (new) % 100 100
...
for i = probs
    ...
    probspecs.m = Var(i, 3);
    if strcmp(mode,'midscale')
        objective = @(x)yatsop_wrapper(x, probspecs, probtype);
    else
        objective = @(x)yatsop_wrapper(x, probspecs, probtype);
    end
    ...
    % set X0
    factor = 10^(0); % revisit!
    X0 = dfoxsnew(probspecs.m, probspecs.n, probspecs.nprob); % starting point
    X0 = factor * X0;
    X0 = X0';
