function [X, F, hF, flag, xk_in] = sam_pounders(Ffun, X_0, n, nf_max, g_tol, delta_0, m, Low, Upp, Prior, Options, Model)
...
% MM: Need to decide which of these data structures to keep, which to boot
% because they're handled by ComponentModel array
...
if nfs == 0 % Need to do the first evaluation
    ...
    for j = 1:m
        ...
        id_tag = j;
        fun = @(x)Ffun(x, id_tag);
        models(j) = ComponentModel(id_tag, fun, X_0, F0, xk_in, np_max, Par, Low, Upp, delta_0, nf_max, 1);
    ...
    for j = 1:m
        ...
        models(j) = ComponentModel(id_tag, fun, X_0(1:nfs, :), F0(1:nfs, :), xk_in, np_max, Par, Low, Upp, delta_0, nf_max, 1);
        % update nf in this scope:
        nf = nf + models(j).nf;        
    end
...
while nf < nf_max
    ...
    % Combine models
    Cres = models.Cres;
    Gres = zeros(n, m); Hres = zeros(n, n, m);
    for j = 1:m
        Gres(:, j) = models(j).Gres;
        Hres(:, :, j) = models(j).Hres;
    end
    % MM: Rethink this. We should be checking hfun at our model estimate of
    % F(x), not at Cres. 
    c = hfun(Cres);
    ...
    % Criticality test
    if ng < g_tol
        % to be implemented, see pounders, apply to each model. 
    end
