...
%
function [np, G, H, valid, Mind, S, Sperp] = yet_another_formquad(B, F, delta, npmax, Pars, xkin, vf)

% --DEPENDS ON-------------------------------------------------------------
% phi2eval : Evaluates the quadratic basis for vector inputs
% qrinsert, svd : Standard internal Matlab/Octave/LAPACK commands

Sperp = []; % by default, Mdir should NOT be populated - only if something went wrong. 
S = [];
...
for aff = 1:2
    ...
    elseif aff == 2 && np < n % Need to evaluate more points, then recall
        ...
        %G = [];
        %H = [];
        error('Should not be reachable in this code. \n');
        % return
    elseif aff == 1 % Output subspace we can actually build a model on
        Sperp = Q(:, np + 1:n)'; 
        S = Q(:, 1:np);
    end
