...
stodars_option.DisplayOutputs = 1;     % 1 to display #iteration, #evaluations, function values and stepsize
                                       % during optimization process, 0 otherwise
...
stodars_option.HistoryFile = 1;        % 1 to generate a .txt history file, 0 else (see stodars_MCestimate.m)
stodars_option.InitStepSize = 1;       % Initial stepsize
stodars_option.LowerBounds = [];       % Lower bounds on the problem variables; set to -Inf in the algorithm
                                       % if no bound is provided
stodars_option.MaxFuncEval = Inf;      % Maximum number of function evaluations (handled by
                                       % stodars_MCestimate.m)
stodars_option.MaxNumberIters = 1000;  % Maximum number of iterations (considered only when
                                       % stodars_option.MaxFuncEval = Inf)
stodars_option.RandomSubspaceMode = 1; % 1 for random subspace variant, 0 for the full space classical
                                       % SDDS using n + 1 directions
stodars_option.SampleSize = 5;         % Sample size for estimate computation (see stodars_MCestimate.m)
stodars_option.SeedValue = [];         % Seed value when stodars_option.FixSeed = 1; equals floor(1+ pi) in
                                       % the algorithm if no value is provided
stodars_option.SolutionFile = 0;       % 1 to generate a .txt file containing the solution of the problem;
                                       % 0 otherwise (see [2] below)
stodars_option.StatsFile = 0;          % 1 to generate a .txt stats file, 0 otherwise (see
                                       % stodars_MCestimate.m)
...
stodars_option.UpperBounds = [];       % Upper bounds on the problem variables; set to +Inf in the algorithm
                                       % if no bound is provided
stodars_option.UsePreviousSamples = 1; % 1 to use available samples in cache for estimates computation;
                                       % 0 otherwise
stodars_option.warning = 0;            % Do not modify
probspecs.Dimension = [];              % Initialization! Problem dimension set to that of the starting point
                                       % in the algorithm
